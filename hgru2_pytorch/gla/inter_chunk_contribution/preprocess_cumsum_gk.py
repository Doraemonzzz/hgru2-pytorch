...
def _fwd_preprocess_cumsum_gk(
    ...
    Q_ptr = (
        Q
        + offset_bh * L * D_MODEL_K
        + offset_c * CHUNK_SIZE * D_MODEL_K
...
class PreprocessCumSum_GK(torch.autograd.Function):
    ...
    def forward(ctx, q, k, gk, normalizer_gk=8, clamp_min=-3):
        ...
        _fwd_preprocess_cumsum_gk[grid](
            ...
            normalizer=normalizer_gk,
            clamp_min=clamp_min,
            D_MODEL_K=D_k,
            num_warps=8 if D_k >= 512 else 4,
        )
...
if __name__ == "__main__":
    ...
    v1 = (
        (torch.randn(B, H, num_chunk, chunk_size, D_K))
        .cuda()
        .to(dtype)
        .requires_grad_(requires_grad)
    )
    v2 = (
        (torch.randn(B, H, num_chunk, chunk_size, D_V))
        .cuda()
        .to(dtype)
        .requires_grad_(requires_grad)
    )
    ...
    if test_gradient:
        ...
        (
            ...
            g_key_last_exp,
            g_value_last_exp,
        ) = PreprocessCumSum_GK.apply(q, v1, v2, g1, g2, 1, 1, -1)

        o = (
            (g_key_cumsum**2).sum()
            + (g_value_cumsum**2).sum()
            + (reduce_key**2).sum()
            + (q_exp**2).sum()
            + (g_value_cumsum_exp**2).sum()
            + (g_key_last_exp**2).sum()
            + (g_value_last_exp**2).sum()
        )
        ...
        (
            ...
            g_key_last_exp2,
            g_value_last_exp2,
        ) = prepare_cumsum(q, v1, v2, g1, g2, 1, 1, -1)
        o = (
            (g_key_cumsum2**2).sum()
            + (g_value_cumsum2**2).sum()
            + (reduce_key2**2).sum()
            + (q_exp2**2).sum()
            + (g_value_cumsum_exp2**2).sum()
            + (g_key_last_exp2**2).sum()
            + (g_value_last_exp2**2).sum()
        )
        o.sum().backward(retain_graph=True)
    ...
    if test_speed:
        ...
        for _ in range(100):
            (
                ...
                g_key_last_exp,
                g_value_last_exp,
            ) = PreprocessCumSum.apply(q, v1, v2, g1, g2)
